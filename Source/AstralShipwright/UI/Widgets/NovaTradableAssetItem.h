...
public:
	...
	void Construct(const FArguments& InArgs)
	{
		...
		[
			SNew(SBox)
			.HeightOverride(FNeutronStyleSet::GetButtonSize("LargeListButtonSize").Height)
			[
				SNew(SOverlay)
				...
				[
					...
					[
						SNew(SImage)
						.Image(InArgs._Dark ? FNeutronStyleSet::GetBrush("Common/SB_Corner_Left") : new FSlateNoResource)
					]
				]
				...
				[
					...
					[
						SNew(SImage)
						.Image(FNeutronStyleSet::GetBrush("Common/SB_Corner_Right"))
		...
		if (Asset.IsValid() && GameState.IsValid())
		{
			switch (GameState->GetCurrentPriceModifier(Asset.Get(), Area.Get()))
			{
				...
				case ENovaPriceModifier::Average:
					return FNeutronStyleSet::GetBrush("Icon/SB_Average");
