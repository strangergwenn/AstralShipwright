...
void SNovaOrbitalMap::Tick(const FGeometry& AllottedGeometry, const double CurrentTime, const float DeltaTime)
{
	...
	{
		FNovaMergedSpacecraftTrajectory(const FNovaSpacecraftTrajectory& Base)
			: FNovaSpacecraftTrajectory(Base)
		{}
	...
	{
		FNovaMergedSpacecraftTrajectory* ExistingTrajectory = MergedTrajectories.FindByPredicate([&](const FNovaMergedSpacecraftTrajectory& OtherTrajectory)
			{
	...
	for (const FNovaMergedSpacecraftTrajectory& Trajectory : MergedTrajectories)
	{
		...
		{
			SpacecraftLocations = AddPartialCircularOrbit(Origin, Trajectory.StartAltitude, Trajectory.StartPhase, 0, Trajectory.EndPhase - Trajectory.StartPhase, Trajectory.Spacecraft, Style);
		}
		else
		{
			SpacecraftLocations = AddTransferOrbit(Origin, Trajectory.StartAltitude, Trajectory.EndAltitude, Trajectory.StartPhase, 0, Trajectory.Spacecraft, Style);
		}
	...
	FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	for (const FNovaBatchedSpline& Spline : BatchedSplines)
	{
		FSlateDrawElement::MakeCubicBezierSpline(
			...
			Spline.WidthOuter * AllottedGeometry.Scale,
		...
		FSlateDrawElement::MakeCubicBezierSpline(
			...
			AllottedGeometry.ToPaintGeometry(),
			Spline.P0, Spline.P1, Spline.P2, Spline.P3,
			Spline.WidthInner * AllottedGeometry.Scale,
			ESlateDrawEffect::None,
			Spline.ColorInner
		);
	}
	...
	for (const FNovaBatchedPoint& Point : BatchedPoints)
	{
		...
		FSlateDrawElement::MakeBox(
			OutDrawElements,
			LayerId,
			AllottedGeometry.ToPaintGeometry(2 * BezierPointRadius, FSlateLayoutTransform(Point.Pos - BezierPointRadius)),
			&WhiteBox,
			ESlateDrawEffect::None,
			Point.Color
		);
...
TArray<FVector2D> SNovaOrbitalMap::AddCircularOrbit(const FVector2D& Origin, float Radius,
...
TArray<FVector2D> SNovaOrbitalMap::AddPartialCircularOrbit(const FVector2D& Origin, float Radius, float Phase, float InitialAngle, float AngularLength,
	const TArray<float>& PointsOfInterest, const FNovaSplineStyle& Style)
{
	FNovaSplineResults Results = AddOrbit(FNovaSplineOrbit(Origin, Radius, Radius, Phase, InitialAngle, AngularLength, 0.0f), PointsOfInterest, Style);
	return Results.PointsOfInterest;
}
	...
	const TArray<float>& PointsOfInterest, const FNovaSplineStyle& Style)
{
	...
	FNovaSplineResults Results = AddOrbit(FNovaSplineOrbit(Origin, MajorAxis, MinorAxis, Phase, InitialAngle, 180, 0.5f * (RadiusB - RadiusA)), PointsOfInterest, Style);
...
FNovaSplineResults SNovaOrbitalMap::AddOrbit(const FNovaSplineOrbit& Orbit, const TArray<float>& PointsOfInterest, const FNovaSplineStyle& Style)
{
